...
```
    ...
    #Change the below function as needed, this takes multiple formulas in the form of a vector, e.g. c("response ~ var1", "response ~ var2 + var3"), and returns a vector of their MSE's, e.g, c(0.3300, 0.5780)
    #Function to do k-fold cross-validation on LINEAR models, given as a vector (or list) of model formulae. 
    #Note that this only returns the CV MSE, not the parameter estimates on each fold.
    #From Advanced Data Analysis from an Elementary Point of View by Cosma Shalizi, Ch. 3, P. 75
