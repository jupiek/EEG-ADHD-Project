...
+ Include both alpha/theta
    ...
    ```
    Scale Data
    + The original dataset that contains the actual connectivity/activity measure of the ADHD v. baseline pairs are normalized, and a modified dataset is made by subtracting the normalized connecivity/activity measure from the normalized baseline. This modified dataset is called *newdf.norm* and it is a substitute for *newdf*.
    + The modified dataset that contains the change of connectivity/activity measures from baseline are normalized as well and saved as *newdf*. It is a substitue for *newdf.norm*.
    + Summary: *newdf.norm* is obtained by normalizing the original data first and then subtracting activity from baseline; *newdf* is obtained by subtracting activity from baseline first and then normalizing the change.
    ```{r}
    #1.1 Scale data to be centered at 0, with standard deviation 1
    ...
    #1.2 
    #Subtract normalized Baseline data from normalized Activity Data 
    #Setup for subtraction
    n = nrow(scaled.data) #sample size (baseline + activity) is 62
    pairs = seq(1,n-1,by=2) #take out odd rows
    newdf.norm = data.frame() #setup a dataframe to store the subtracted number (i.e. change from baseline)
    temp = scaled.data[,-c(1,2)] #setup a temperary dataset that drops the ID and the ADHD column
    rowID = as.character(scaled.data[pairs,1]) #save the ID to rowID
    #Subtract the EEG measures by every 2 rows
    for(i in pairs){
      newdf.norm = rbind(newdf.norm, temp[i,]-temp[i+1,])
    }
    #Add subject ID and ADHD diagnostic back for future reference
    newdf.norm = data.frame(ID = rowID, ADHD = rep(1,nrow(newdf.norm)),newdf.norm)
    #Enforce consecutive reference of rows
    rownames(newdf.norm)=1:nrow(newdf.norm)
    
    #2.1 Normalize the change dataset "newdf" (the change of connectivity/activity from baseline)
    scaled.newdf = as.data.frame(cbind(ID = newdf[,1], ADHD = newdf[,2], scale(newdf[,-c(1,2)])))
    #Check that dataset is normalized
    checkMat = round(colMeans(scaled.newdf[,-c(1,2)]),10)==0
    ifelse(length(checkMat[checkMat==TRUE])/length(checkMat)==1,
          print("Data to be centered at 0 success"),
          print("Data to be centered at 0 fail"))
    checkMat2 = round(apply(scaled.newdf[,-c(1,2)], 2, sd),10)==1
    ifelse(length(checkMat2[checkMat2==TRUE])/length(checkMat2)==1,
          print("Data to have standard deviation of 1 success"),
          print("Data to have standard deviation of 1 fail"))
