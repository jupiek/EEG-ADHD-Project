...
plot(prvar.norm, xlab="Principal Component", ylab ="",
     main = "Prop. of Var.")
plot(cumsum(prvar.norm), xlab="Principal Component", ylab ="",
     main = "Cum. Prop. of Var.")
screeplot(prout.norm, type="lines", npcs=71, main = "Screeplot")
...
plot(prvar.change.norm, xlab="Principal Component", ylab ="",
     main = "Prop. of Var.")
plot(cumsum(prvar.change.norm), xlab="Principal Component", ylab ="",
     main = "Cum. Prop. of Var.")
screeplot(prout.change.norm, type="lines", npcs=71, main = "Screeplot")
...
plot(prvar.ACT.norm, xlab="Principal Component", ylab ="",
     main = "Prop. of Var.")
plot(cumsum(prvar.ACT.norm), xlab="Principal Component", ylab ="",
     main = "Cum. Prop. of Var.")
screeplot(prout.ACT.norm, type="lines", npcs=71, main = "Screeplot")
...
plot(prvar.CON.norm, xlab="Principal Component", ylab ="",
     main = "Prop. of Var.")
plot(cumsum(prvar.CON.norm), xlab="Principal Component", ylab ="",
     main = "Cum. Prop. of Var.")
screeplot(prout.CON.norm, type="lines", npcs=71, main = "Screeplot")
...
plot(prvar.ATT.norm, xlab="Principal Component", ylab ="",
     main = "Prop. of Var.")
plot(cumsum(prvar.ATT.norm), xlab="Principal Component", ylab ="",
     main = "Cum. Prop. of Var.")
screeplot(prout.ATT.norm, type="lines", npcs=71, main = "Screeplot")
...
plot(prvar.alpha.norm, xlab="Principal Component", ylab ="",
     main = "Prop. of Var.")
plot(cumsum(prvar.alpha.norm), xlab="Principal Component", ylab ="",
     main = "Cum. Prop. of Var.")
screeplot(prout.alpha.norm, type="lines", npcs=71, main = "Screeplot")
...
plot(prvar.theta.norm, xlab="Principal Component", ylab ="",
     main = "Prop. of Var.")
plot(cumsum(prvar.theta.norm), xlab="Principal Component", ylab ="",
     main = "Cum. Prop. of Var.")
screeplot(prout.theta.norm, type="lines", npcs=71, main = "Screeplot")
```
...
####Elastic Net (Lasso and Ridge Regression Combined)
    ...
    test_error <- function(data){
      ...
      for(i in 1:100){
        ...
        #cross validation
        cvfit <- cv.glmnet(y=train$ADHD, x=as.matrix(train[, -c(1,2)]), family="binomial", alpha=0.7, type.measure = "class", nfolds=5)
        mis_rate[i] <- mean(predict(cvfit, s="lambda.min", type="class", newx = as.matrix(test[, -c(1,2)])) != test$ADHD)
    ...
    mean(mis_rate_norm)
    mean(mis_rate_change_norm)
    mean(mis_rate_act_norm)
    mean(mis_rate_con_norm)
    mean(mis_rate_att_norm)
    mean(mis_rate_alpha_norm)
    mean(mis_rate_theta_norm)
    ```
    ...
    er.fit = glmnet(x = as.matrix(data.change.norm[,-c(1,2)]), 
                    y = data.change.norm$ADHD, family = "binomial", alpha = 0.7)
    
    cv.fit = cv.glmnet(x = as.matrix(data.change.norm[,-c(1,2)]), 
                       y = data.change.norm$ADHD, family = "binomial", 
                       alpha = 0.7, type.measure = "class", nfolds = 5)
    
    #The optimal value of λ, by 5-fold cross-validation, is 0.032. 
    ...
    #Function load.cis()
    #Returns the a-level CI of a factor model's loadings - first factor only
    ...
    #Output
      #Plot of the factor loadings with confidence intervals denoted by error bars
    
    load.cis = function(df, f = 1, a = 0.90) {
      
      f.mdl = factanal(df, factors = f)
      
      resample = function() {resample.data.frame(df)}
          
      estimator = function(data) {
        mdl = factanal(data, factors = f)
        lgs = mdl$loadings[,1]
        return(lgs)
      }
      
      lgs = f.mdl$loadings[,1]
      
      load.cis = bootstrap.ci(statistic = estimator,
                              simulator = resample, 
                              B = 1000, t.hat = lgs, 
                              level = a)
      
      loadings = sort(lgs)
      names = rownames(lgs)[order(lgs)]
      bp = barplot(loadings, names.arg = names, las = 2, 
                   ylim = c(0, max(load.cis)+0.05),
                   main = "Factor 1 Loadings", 
                   sub = paste("*confidence intervals denoted in red; level =", a), 
                   ylab = "Loadings")
      error.bar(bp, loadings, load.cis[,1][order(lgs)], load.cis[,2][order(lgs)])
...
+ Who is our advisor? Supervisor pef, advisors Cassie and Kevin?
+ Alphabetical, *all first authors
