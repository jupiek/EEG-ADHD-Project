...
####Read in, Clean up
    ...
    # Just EEG Activity
    data.ACT = data[,-c(43:84)]
    data.ACT
    dim(data.ACT)
    
    # Just EEG Connectivity
    data.CON = data[,c(1:2,43:86)]
    data.CON
    dim(data.CON)
    
    # Just EEG Connectivity + Activity without baseline
    data.ATT = data[,c(1,2,23:42,63:82,85,86)]
    data.ATT
    dim(data.ATT)
...
####Elastic Net (Lasso and Ridge Regression Combined)
    ...
    cvfit <- cv.glmnet(y=data$ADHD, x=as.matrix(data[, -c(1,2)]), family="binomial", alpha=0.7, type.measure = "class", nfold=5)
    cvfit$lambda.min
    #the largest λ at which the MSE is within one standard error of the minimal MSE.
    cvfit$lambda.1se
    #lambda which returns the minimum MSE
    cvfit$lambda.min
    plot(cvfit)
    
    #Plot of probabilities vs. observations
    plot(data$ADHD, predict(cvfit, newx = as.matrix(data[,-c(1,2)]), type = "response"))
    ```
